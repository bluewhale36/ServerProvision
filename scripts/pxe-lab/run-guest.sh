...
BIOS_VER="${BIOS_VER:-F13}"
if [ "${PLACEHOLDER:-0}" = "1" ]; then
    BOARD_SERIAL="To Be Filled By O.E.M."   # placeholder 필터(V8 대비) 검증 변형
else
    BOARD_SERIAL="${BOARD_SERIAL:-JG4P6400027}"
fi
...
set -- \
    ...
    -smbios "type=2,manufacturer=$VENDOR,product=$BOARD,serial=$BOARD_SERIAL" \
