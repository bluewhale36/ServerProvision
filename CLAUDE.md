...
- 동시 작업은 `claude --worktree` 로 워크트리 분리. `.worktreeinclude` 가 `CLAUDE.local.md`(빌드·DB env)를 새 워크트리로 승계하고, 누적 메모리는 저장소 기준이라 워크트리끼리 공유된다. `ServerProvision-dev` 는 `.git` 실체를 보유한 앵커라 **이동·삭제 금지**.
