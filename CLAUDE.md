...
### 체크포인트 (CP1~CP7)
12 단계를 7 체크포인트로 묶어 **단계별 사용자 승인 후 다음으로** 진행한다(승인 전 선행 구현 금지).
...
- **먼저 검증 항목을 계획한다.** 무엇을 확인할지 목록을 만들어 제시하고 시작한다. 즉흥적으로 클릭하며 훑지 않는다 — 계획이 있어야 빠진 것이 보이고 CP6 의 보고가 성립한다.
...
- **스크린샷으로 채증한다.** 각 항목의 결과를 화면으로 남긴다. 이 증거가 CP6 report 의 본문이 된다.
...
- **report** = **CP6(Step 11) 산출물**(캠페인·단계 완료 보고에도 같은 규약을 쓴다). `report/YY-MM-DD_HH-MM-SS_<단계명>_report.html`. plan 과 같은 명세·골격을 따르되 섹션 구성은 보고 대상에 맞춘다. **CP5 샌드박스 검증의 기록이 본문의 중심**이다 — ① 무엇을 확인하려 했는지(검증 항목 계획) ② 실제로 어떤 조작을 했는지(정상·비정상 시도 각각) ③ 스크린샷 증거 ④ 검증 중 발견한 결함과 그 처리. 구현 요약만 있고 검증 기록이 없는 report 는 CP6 승인 대상이 아니다.
- **report 의 기준 구현 = `report/26-08-07_15-45-05_S10_report.html`**(사용자가 분량·밀도·전달력을 승인한 형태). 골격은 명세를 따르되 **섹션 구성과 서술 밀도는 이것을 본다**. 특징 — 진단(실측 근거) → 구현 → 강제 장치 → CP5 계획 → CP5 수행 기록 → **미수행 항목과 사유** → 발견한 것 → Critical Files → 남은 일. 항목 식별자 22 개(A1~A10 · B1~B8 · C1~C4)에 스크린샷 16 장이 1:1 로 붙고, 본문은 1 만 자 남짓이다. **못 한 것을 사유와 함께 적는 §7 이 이 보고서의 핵심**이다 — 빠진 것을 감추지 않아야 보고가 성립한다. 정본 952 KB / 임베드본 44 KB.
- **스크린샷과 Notion 임베드의 충돌(실측)**: 스크린샷을 data URI 로 인라인하면 report 가 수백 KB~수 MB 가 되어 **Notion 임베드가 불가능**하다(한국어 html 은 약 70KB 가 상한). 그래서 **두 벌을 낸다** — 스크린샷을 담은 **정본**(`_report.html`, 저장소 보관)과 스크린샷을 뺀 **임베드본**(`_report-notion.html`). 임베드본도 70KB 를 넘으면 자체 완결 파트로 분할해 여러 embed 로 넣는다. 정본을 Notion 에 올리려 시도하지 않는다 — 잘린 첨부는 손상된 embed 로 남아 없느니만 못하다.
