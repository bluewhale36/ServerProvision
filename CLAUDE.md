...
### 브랜치 운용 (2026-07 재편)
- 3계층: **`main`**(실 배포 소스, GitHub 기본 브랜치) ← **`dev`**(개발 완료 도달점) ← **`feature/<type>-<인벤토리코드>-<슬러그>`**(작업). type 은 conventional 어휘(`feat`/`fix`/`refactor`/`docs`/`chore`). 예: `feat/E1-diagnose-linux`.
- feature 브랜치는 **캠페인/phase 단위**(plan 1건마다 파지 않는다). PR 은 CP4(테스트 green)에서 열고, CP5 사용자 완료 통보 후 `dev` 로 `--no-ff` 병합. `dev`→`main` 은 배포 시 병합.
...
- 동시 작업은 워크트리로 분리. **생성 표준 = 수동** `git worktree add -b <feat/코드-슬러그> <base>/ServerProvision-<브랜치> origin/dev` — 폴더 경로 = `ServerProvision-<브랜치>`(브랜치의 `/` 를 경로로 보존, 저장소 기존 관례: `ServerProvision-renew/main`·`ServerProvision-feature/setting`). 편의 함수 `~/.zshrc: spv-wt <브랜치>`. (내장 `claude --worktree` 는 브랜치를 워크트리 이름으로 만들어 슬래시 규약명이 안 나오고 base 가 main 고정이라 쓰지 않는다.) `CLAUDE.local.md`(gitignore)는 생성 후 `cp` 로 승계, 누적 메모리는 저장소 기준이라 워크트리끼리 자동 공유. `ServerProvision-dev` 는 `.git` 실체를 보유한 앵커라 **이동·삭제 금지**.
