...
| 새 안내 문구 · 화면 라벨 | `new-user-copy.md` |
...
작업은 **인벤토리 코드**(작업 단위 식별자)로 부른다: `MA*`(Manage-Application) / `MK*`(Manage-Kernel/Maintenance) / `U*`(Provisioning) / `E*`(Execution — 프로비저닝 실행 엔진, E1=진단 리눅스부터 `ProvisioningPhase` 순서 대응) / `S*`(cross-cutting infra) / `R*`(리팩토링 캠페인) / `HF*`(hotfix) / `M0`(리네임) / `CH*`(housekeeping) / `DOC*`(문서화) / `OPS*`(서버 운영 설계 — 배포 대상 OS 위에서 애플리케이션을 어떤 계정·권한·배치로 돌릴지) / `DPL*`(Deploy — 배포 실행·기록. OPS 가 정한 운영 설계를 특정 환경에 실제 배포·검증한다. DPL-1=스테이징 VM 테스트 배포, DPL-2=실배포. 산출물이 코드가 아니므로 CP 체계 비적용). 코드 번호는 식별자이지 실행 순서가 아니다. **각 코드의 상태·이력은 Notion DB 가 SSOT** — CLAUDE.md 에 이력을 적지 않는다.

### 수직 슬라이스 (페이지/작업당 12 단계)
1. URL/데이터 흐름 스케치 — **plan html 산출**(아래 규약) 2. Thymeleaf 뷰(더미, 기존 CSS 재사용) 3. Controller(`@ModelAttribute`+`BindingResult`, Model 엔 Response 만) 4. Request/Response DTO(`@Valid`) 5. Service 인터페이스+시그니처(`@Transactional` 경계) 6. Repository(Spring Data 네임규칙) 7. Entity(`BaseTimeEntity` 상속, **7단계 전 `@Entity` 작성 금지**) 8. Service 본체 + 테스트(아래 규율) 9. 스키마 확인(`ddl-auto=validate` — 수동 DDL 을 `ddl/` 에 산출·적용 후 `SHOW CREATE TABLE` 검증. `claude_code` 계정이 ALTER · CREATE 를 보유해 직접 적용 가능 — DELETE · DROP 은 없음, 2026-08-23 SHOW GRANTS 실측) 10. **샌드박스 브라우저 검증**(아래 규율) 11. **report html 산출**(아래 규약) 12. 브라우저 E2E — **사용자 단독**.
- 리팩토링 슬라이스(엔티티 무변경)는 6·7·9 단계가 N/A 가 될 수 있다. 화면이 없는 슬라이스는 10 단계를 하네스 검증으로 대체한다(아래 E 단계 예외).

### 체크포인트 (CP1~CP7)
12 단계를 7 체크포인트로 묶어 진행한다. **사용자 승인 게이트는 CP1 · CP4(중간 승인) · CP6 · CP7 넷이다** (2026-08-12 확정). 운용은 "Step 마다 승인 → CP 마다 승인 → 일부 무중단" 으로 진화해 왔는데, 승인 지점이 문서에 없어 세션마다 plan 에 제각각 오기입하던 것을 여기로 고정한다. 무중단 구간은 둘이다 — **① CP1 승인 → CP2~CP4 게이트 지점**(main 구현, 경계마다 보고만) **② CP4 승인 → CP6 제출**(테스트 작성 · CP5 검증 · report 산출). CP4 를 **중간 승인**으로 두는 이유: 테스트 코드를 쓰기 전에 main 구현을 검수받아야 검수 피드백이 이미 작성한 테스트를 무효화하지 않는다. CP5 는 수행 결과를 보고만 하며 그 검수는 CP6 에서 report 와 함께 받는다(CP1 승인 전 선행 구현 금지는 불변). 각 CP 의 수행 주체와 위임 분업은 아래 블록을 따른다.

| CP | 범위 | 동작 | 게이트 |
|---|---|---|---|
| **CP1** | Step 1 | Claude 가 plan html 생성·제시 | **승인 — plan 내용 검수** |
| **CP2** | Step 2~5 | 뷰·Controller·DTO·Service 시그니처 구현 | 보고 (무중단) |
| **CP3** | Step 6~7 | Repository·Entity(+VO/Enum) | 보고 (무중단) |
| **CP4** | Step 8~9 | Service 본체·스키마 확인 — **테스트 코드 제외한 main 구현 완료 + 빌드 통과(기존 테스트 회귀 없음)** 시점이 게이트 | **중간 승인 — main 코드 검수** |
| **CP4 잔여** | Step 8 | (승인 후) 단위·통합 테스트 작성 + 전체 테스트 green | 무중단 — CP5 로 계속 |
| **CP5** | Step 10 | **샌드박스 브라우저 검증** — 검증 항목 계획 후 수행, 스크린샷 채증 | 보고 (검수는 CP6 에서 report 와 함께) |
| **CP6** | Step 11 | **report html 산출** — CP5 의 계획·수행 기록·증거를 담아 제출 | **승인 — 샌드박스 검증 통과 + report 내용 검수** |
| **CP7** | Step 12 | 사용자 단독 브라우저 E2E | **승인 — E2E 통과 (완료 통보 대기)** |
...
> - **CP2~CP4** — 승인된 plan 범위의 구현은 **메인 세션이 직접, 무중단으로** 진행한다(CP2 · CP3 은 경계 보고만). **CP4 는 중간 승인 게이트다** — 테스트 코드를 제외한 main 구현이 끝나고 빌드가 통과(기존 테스트 회귀 없음)한 시점에 제시해 코드 검수 승인을 받는다. **승인 후 테스트 코드 작성 · 전체 테스트 green → CP5 → CP6 제출까지가 두 번째 무중단 구간**이다.
...
### essential report html 규약 (2026-09-17 확정, 불가침)
- **essential** = 한 슬라이스의 **구조 · 책임 · 흐름** 문서. 정규 CP6 report 가 "무엇을 검증했고 결과가 어땠나" 라면, essential 은 "무엇을 만들었고 · 어떤 구조이며 · 각 클래스가 무엇을 맡고 · 요청 한 건이 어떻게 흐르는가" 만 담고 **검증 방식 · 결과 · 다음 작업은 쓰지 않는다**. 정규 report 를 대체하지 않으며, CP6 뒤 사용자가 지시할 때(클래스가 여럿 얽힌 슬라이스) 낸다. 파일 `report/YY-MM-DD_HH-MM-SS_<인벤토리코드>_essential.html`(KST).
- **저작 SSOT = `.claude/essential-html-spec.md`, 기준 구현(승인본) = `report/26-09-14_23-08-12_S19-1_essential.html`**, 도식 생성기 정본 = `.claude/tools/dgkit.py`. 절 5 개 · id 고정: §1 범위 · §2 구조(부품 배치 · **패키지 트리 = 텍스트 트리** · 타입 사슬 · 정적 산출물) · §3 데이터 흐름(시퀀스 · 거절 분기) · §4 클래스별 책임(맡는 것 · 맡지 않는 것 4 열 표) · §5 계약.
- **그래픽은 흐름에만**: 패키지 · 디렉토리 트리는 `pre.pkg-tree` 텍스트 트리로 쓰고, 트리형 그래픽(`tree_svg`)은 코드의 흐름(필터 체인 · 호출 사슬 · 분기)을 트리로 보일 때만 붙인다. 도식은 인라인 SVG(`figure.dg` + figcaption) · 이미지 파일 없음 · 색은 `:root` 토큰만 · JS 4 동작(목차 스크롤 · 전부 펴기/접기 · 필터 · check-list 보존)은 승인본과 동일.
- 분업은 plan · report 와 같다 — 내용(절 본문 · 레인 · 메시지 · 트리 행 · 책임 표 · 계약)은 세션이 브리프로 쓰고, html 조립 · SVG 생성 · Playwright 렌더 확인(1360 · 820 · 390 폭 · overflow 0)은 Opus 하위 에이전트에 위임하며 세션이 검수한다.
...
- **현행 스트림 셋.**
  ...
  - **`ServerProvision-design-experiment` (디자인)** — 화면 디자인 · 프론트 정비 본류(2026-09-06 승격). 2026-08-18 단기 실험 워크트리로 시작했으나 방향안 C 채택 뒤 승격 슬라이스가 여러 캠페인(토큰 흡수 → 마크업 조각화 → 페이지 CSS 흡수 → JS 공통화 → CDN 로컬화)에 걸쳐 이어지고 9 차례의 디자인 판정 맥락이 재사용되므로 3 조건을 채웠다. **디렉터리 이름은 그대로 둔다** — 세션 맥락이 절대경로에 묶여 있어 옮기면 그 맥락이 끊어진다(이름과 역할이 어긋나는 것은 감수). 실험은 `refine/design-experiment`(로컬 전용 · push 금지)에서, **승격은 `origin/dev` 에서 딴 정규 브랜치(`refactor/S17-<n>_…`)**로 PR 을 낸다 — 실험 시트를 얹는 것이 아니라 원본 CSS · 템플릿을 고치는 것이 승격이다. Notion 우산 = `S17 : 디자인 승격(방향안 C)`. 샌드박스는 7797(데이터는 hf11 DB 3379 재사용).
- **스트림 신설 3 조건 — 전부 충족할 때만 만든다.** ① 그 영역의 작업이 **여러 캠페인에 걸쳐 지속**되고 ② 다른 스트림과 **동시에 진행**되어야 하며 ③ 축적되는 **도메인 맥락이 재사용**된다. 하나라도 미달이면 만들지 않는다(DOC·학습은 ① 이 아직 없어 보류 · 디자인은 2026-09-06 에 셋을 채워 승격). "미리 분리 금지" 원칙의 세션 판이다.
