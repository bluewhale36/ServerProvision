...
### 체크포인트 (CP1~CP7)
12 단계를 7 체크포인트로 묶어 진행한다. **사용자 승인 게이트는 CP1 · CP4 · CP6 · CP7 넷이다** (2026-08-12 확정). 운용은 "Step 마다 승인 → CP 마다 승인 → 일부 무중단" 으로 진화해 왔는데, 승인 지점이 문서에 없어 세션마다 plan 에 제각각 오기입하던 것을 여기로 고정한다. **나머지 CP 는 무중단 구간**이다 — CP1 승인이 CP2~CP3 의 진행 승인을 겸하고(경계마다 보고만), CP5 는 수행 결과를 보고만 하며 그 검수는 CP6 에서 report 와 함께 받는다(CP1 승인 전 선행 구현 금지는 불변). 각 CP 의 수행 주체와 위임 분업은 아래 블록을 따른다.
...
| **CP4** | Step 8~9 | Service 본체·테스트·스키마 확인 | **승인 — 코드 검수 · 빌드/테스트 green 확인** |
...
> - **CP2~CP4** — 승인된 plan 범위의 구현 · 테스트 · 스키마 확인은 **메인 세션이 직접, 무중단으로** 진행한다(CP2 · CP3 은 경계 보고만). **CP4 는 승인 게이트다** — 빌드 · 전체 테스트 green 을 확인해 제시하고 코드 검수 승인을 받은 뒤 CP5 로 넘어간다.
