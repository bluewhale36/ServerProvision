...
- **상주 스트림 = 워크트리 1 개 = 세션 홈.** 디렉토리명은 `ServerProvision-<스트림>`. 캠페인이 바뀌어도 **디렉토리를 새로 만들지 않고** 그 안에서 `git switch -c <type>/<인벤토리코드>_<슬러그> origin/dev` 로 브랜치만 갈아끼운다. 세션은 그대로 이어져 직전 캠페인의 맥락 위에서 다음을 시작한다.
- **현행 스트림 둘.**
