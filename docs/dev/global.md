...
두 어드바이스의 분리는 내용 협상(content negotiation)이 담당한다. 둘 다 최우선 순위이고, JSON 핸들러는 `produces`가 JSON, HTML 핸들러는 HTML이다.

그런데 이 협상만으로는 브라우저에서 HTML 핸들러가 선택되지 않는다. 어드바이스 선택은 Accept 의 선호도 값을 보지 않고, 등록 순서대로 훑으면서 요청이 받아들일 수 있는 형식이면 채택하고 멈춘다. 브라우저는 Accept 에 모든 형식을 허용하는 항목을 항상 덧붙이므로 그 항목이 JSON 핸들러와 먼저 맞아떨어진다. 그래서 `DocumentNavigationAcceptFilter`가 문서 이동으로 들어온 요청의 Accept 를 `text/html`로 좁혀 보고한다. 판별 기준은 `Sec-Fetch-Dest` 헤더가 `document`인지 여부이며, `fetch`와 비브라우저 클라이언트는 손대지 않으므로 종전대로 JSON 을 받는다. 배경과 검토한 대안은 [ADR 0001](adr/0001-error-response-channel.md)에 있다.

> 주의: 두 어드바이스의 `@Order` 값에 최우선 상수 기준의 덧셈이나 뺄셈을 하면 안 된다. 뺄셈은 정수 언더플로로 사실상 최하위가 되어 무효였고, 덧셈은 모호한 Accept 케이스의 회귀를 일으켰다. 두 값 모두 최우선 상수 그대로 둔다. 채널이 잘못 선택되는 문제는 순서가 아니라 협상의 입력을 바로잡아 해결한다.
...
브라우저 내비게이션이면 `DocumentNavigationAcceptFilter`가 Accept 를 좁혀 둔 덕분에 3에서 HTML 핸들러가 선택되고, 같은 예외가 오류 페이지가 된다. 필터가 없으면 이 선택이 이루어지지 않는다는 점이 중요하다. 자세한 내용은 [ADR 0001](adr/0001-error-response-channel.md)을 본다.

어드바이스가 아예 잡지 못하는 예외, 예를 들어 매핑이 없는 주소나 필수 파라미터 누락은 기본 오류 경로로 흐른다. 이 경로는 평범한 컨트롤러 매핑을 타서 Accept 의 선호도를 제대로 지키므로 브라우저에는 처음부터 HTML 로 응답해 왔다. 상관관계 필터는 그 과정에서 미처리 예외를 최후 안전망으로 기록한 뒤 다시 던진다.

### 폼 제출과 오류 표시

서버가 형식을 옳게 고르는 것만으로는 부족하다. 브라우저가 폼을 기본 제출로 보내면 페이지가 이동하므로, 응답이 무엇이든 그것이 새 화면이 된다. 그래서 상태를 바꾸는 폼은 페이지를 이동시키지 않아야 한다.

`static/global/form-submit.js`가 그 역할을 한다. 레이아웃에서 전역으로 적재되며, 제출 이벤트를 받아 `fetch`로 보내고 응답을 `window.AsyncSubmitResult`에 위임한다. 거절이면 전역 오류 모달이 사유를 표시하고 페이지는 그대로 유지된다.

가로채기가 기본이고 빠져나가는 쪽이 예외다. 종전에는 폼마다 마커를 붙여야 `fetch`로 나갔는데, 마커를 잊는 것이 곧 응답 본문 노출 사고였다. 기본값을 뒤집어 잊었을 때의 결과가 안전해지게 했다.

빠져나가는 폼은 `data-native-submit` 속성에 사유를 적는다. 대상은 실패 시 뷰를 다시 렌더하는 폼, 즉 핸들러가 `BindingResult`를 받는 경우다. 이런 폼을 가로채면 서버가 돌려준 재렌더 결과를 버리게 되어 필드별 오류 표시가 사라진다.

페이지 전용 스크립트가 자기 방식으로 처리하는 폼은 표시가 필요 없다. 전역 인터셉터는 이미 기본 동작이 취소된 제출을 건드리지 않으므로, 업로드나 재확인처럼 자체 흐름을 가진 화면은 그대로 동작한다.

> 주의: 새 입력 폼을 추가하면서 `data-native-submit` 표시를 빠뜨리면 `FormSubmissionConventionTest`가 실패한다. 재렌더 핸들러 수와 표시된 폼 수가 어긋나기 때문이다. 이 관계를 정당하게 깨는 변경이라면 사유와 함께 기대값을 고친다.
