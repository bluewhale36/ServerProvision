...
- [ ] Redfish SimpleUpdate 로 커스텀 BIOS 파일이 적용되는지 (E3-R 조사의 실기 확인 항목). **→ 2026-08-19 부분 실측(원장 = Notion E0-4)**: 표준 `image.RBU` 는 `UpdateComponent: "BIOS"` 로 동일 버전(F27, 47초) · 상위 버전(F29, 2분 22초) 모두 flash 성공 — 단 **1영역(`BIOS`)만 갱신되고 `BIOS2` 는 버전 미노출로 거동 불명**. **커스텀 BIOS 이미지는 RBU 경로에 반영되지 않고 기본 이미지로 flash 됨(실측)** — 벤더 문의 후 방향 재조정 필요, 이 항목은 그때까지 미결 유지. 2영역 갱신은 `UpdateComponent` 허용값의 `HPM_BIOS2` 가 후보(미실측).
