...
| **CP1 정정(09-03)** | **E4-1-a-3 CP1 승인 시 사용자 결정 — 설치 시한 60분 · 재진입 상한 5회**(권고 90분 · 6회에서 축소). D1 의 "3회째 HOLD" 는 plan 이 정정했다 — 착수 뒤에는 결손 대기가 거부되므로(STEP_RUNNING) 재PXE 는 시한 · 상한 초과만 FAILED 로 전환한다. ComputerName `SPV-<systemUUID 뒤 8 hex>` · 리눅스 정의서는 BLOCKED 대기(TTL) 는 권고안 그대로 |
