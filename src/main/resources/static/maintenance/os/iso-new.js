/* ============================================================
   maintenance/os/iso-new.html 전용 스크립트
   ─────────────────────────────────────────────────────────────
   ISO 생성 정책 :
     · 파일 없음 → background job 도, XHR 도 쓰지 않는다. 기본 form submit 허용 →
       MVC POST /iso 가 즉시 처리되고 redirect. 수 ms 내 끝나므로 UI 잠금도 불필요.
     · 파일 있음 → foreground XHR 로 업로드. 실시간 % / 속도 / ETA 는 페이지 내 진행 바에
       표시되고, 업로드 중엔 navbar 링크 · "← 목록으로" 링크를 잠가 실수로 페이지를
       벗어나 XHR 이 cancel 되는 사고를 방지한다. 취소 버튼은 XHR abort 로 유지.
...
(function () {
    ...
    form.addEventListener('submit', async e => {
        ...
        if (!file) {
            console.log(TAG, '파일 없음 — 기본 submit 허용');
            return;
        }
    ...
    async function requestIntent(isoPath, file, allowCreateDirectory) {
        // uploadUrl = /pxe/v1/maintenance/os/{osId}/iso/upload → 같은 경로에서 /upload-intent 로 파생
