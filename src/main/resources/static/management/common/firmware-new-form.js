/* ============================================================
   firmware-new-form.js — BIOS · BMC 신규 등록 폼 스크립트 한 벌 (S17-4)
   ─────────────────────────────────────────────────────────────
   금지 파일명 사전 검사 · Intent 사전검증(업로드 시) · XHR foreground 업로드 · 파일 없는 기존 파일
   등록(claim) · MK2 nudge 모달 두 단계. bios-new.js 와 bmc-new.js 가 779 줄 중 22 줄(폼 id · 바운드
   플래그 · nudge 조각 prefix · 로그 태그)만 다른 복사본이어서 규칙을 바꾸면 양쪽을 함께 고쳐야 했다.
   그 22 줄을 설정 객체로 받고 본문은 여기 한 벌만 둔다. iso-new.js · subprogram-new.js 는 뼈대만
   비슷하고 본문이 갈려(각 1,100 줄 이상 차이) 합치지 않는다.

   create({tag, formId, boundFlag, nudgePrefix}) → {init}
     · tag         — console 로그 태그('[bios-new]')
     · formId      — 폼 요소 id('biosForm')
     · boundFlag   — 같은 폼 인스턴스에 중복 바인딩을 막는 dataset 키('biosNewBound')
     · nudgePrefix — nudge 모달 조각(fragments/management/nudge-modal)의 prefix('biosNudge' → #biosNudgeModal …)
     · formUrl     — 보드 id → 그 보드의 등록 폼 조각 URL(boardId 없이 들어온 화면의 #boardSelector 가 쓴다)
   S5-5 — 외부 우상단 '+ 신규 등록' 진입 시 폼이 AJAX 로 동적 삽입되므로 init() 은 DOMContentLoaded 와
   폼 주입 뒤 둘 다에서 부를 수 있어야 한다(idempotent).
   ============================================================ */
(function () {
    function create(cfg) {
        const TAG = cfg.tag;
        const nudgeEl = suffix => document.getElementById(cfg.nudgePrefix + suffix);

        /* S5-5 — 보드를 먼저 고르는 진입(boardId 없음)에서는 #boardSelector 의 change 마다 그 보드의 폼을 AJAX 로 받아
           #formCardSlot 에 넣고 init() 을 다시 부른다. 종전에는 두 템플릿이 같은 23 줄을 인라인으로 들고 있었다
           (S17-4 CP5 F-2). 두 요소가 없는 화면(boardId 로 바로 들어온 경우)에서는 아무 일도 하지 않는다. */
        function bindBoardSelector() {
            const selector = document.getElementById('boardSelector');
            const slot = document.getElementById('formCardSlot');
            if (!selector || !slot || selector.dataset.firmwareNewBound === '1') return;
            selector.dataset.firmwareNewBound = '1';
            selector.addEventListener('change', async function () {
                ...
                try {
                    const resp = await fetch(cfg.formUrl(boardId), {headers: {'X-Requested-With': 'XMLHttpRequest'}});
                    if (!resp.ok) {
                        console.error(TAG, 'form fetch failed:', resp.status);
                        return;
                    }
                    slot.innerHTML = await resp.text();
                    init();
                } catch (err) {
                    console.error(TAG, 'form fetch error:', err);
                }
            });
        }
        ...
        function init() {
            ...
            if (!form) return;
            if (form.dataset[cfg.boundFlag] === '1') return;
            form.dataset[cfg.boundFlag] = '1';
            ...
            function openNudgeModal(body) {
                const modal = nudgeEl('Modal');
                const conflicts = nudgeEl('ConflictsList');
                const proceedBtn = nudgeEl('ProceedBtn');
                const replaceBtn = nudgeEl('ReplaceBtn');
                const cancelBtn = nudgeEl('CancelBtn');
                ...
                (body.conflicts || []).forEach(entry => {
                    ...
                    li.innerHTML =
                        ...
                        '  <input type="radio" name="' + cfg.nudgePrefix + 'Target" value="' + entry.id + '">' +
                ...
                conflicts.querySelectorAll('input[name="' + cfg.nudgePrefix + 'Target"]').forEach(input => {
            ...
            function openIntentNudgeModal(body, fields, file) {
                ...
                (body.conflicts || []).forEach(entry => {
                    ...
                    li.innerHTML =
                        '<label style="display:flex; gap:8px; align-items:center; cursor:pointer;">' +
                        '  <input type="radio" name="' + cfg.nudgePrefix + 'IntentTarget" value="' + entry.id + '">' +
                        '  <span><strong>' + UiUtil.escapeHtml(entry.name) + '</strong> · v' + UiUtil.escapeHtml(entry.version) +
                        '    <span style="color: var(--n-text-muted, #777); font-size: 11px;">[' + entry.state + ' · #' + entry.id + ']</span></span>' +
                        '</label>';
                    conflicts.appendChild(li);
                ...
                conflicts.querySelectorAll('input[name="' + cfg.nudgePrefix + 'IntentTarget"]').forEach(input => {
                    input.addEventListener('change', () => {
                        selectedTargetId = input.value;
                        replaceBtn.disabled = false;
                    });
            ...
            function disableNudgeButtons(disabled) {
                ['ProceedBtn', 'ReplaceBtn', 'CancelBtn'].forEach(suffix => {
                    const el = nudgeEl(suffix);
    ...
    window.FirmwareNewForm = {create: create};
})();
