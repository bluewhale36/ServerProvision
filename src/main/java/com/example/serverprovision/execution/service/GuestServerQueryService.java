...
public class GuestServerQueryService {
    ...
    /**
     * OS 가시 디스크(lsblk) 행 — 조립 규칙은 {@link OsVisibleDiskRows} 가 SSOT(2026-09-17). RAID 볼륨과 짝이 맞는 장치는
     * 볼륨 이름과 함께 <b>멤버 물리 디스크</b>의 종류 · 전송(CLI 보고)을 보인다 — lsblk 의 tran 은 카드가 내미는 전송(SAS)이라
     * SATA SSD 가 SAS 로 보이던 것(CRA3338 · 9361-8i)의 정정. 수집 불가 값은 띄우지 않는다는 사용자 지시는 유지.
     */
    private List<GuestServerDetailResponse.OsVisibleDisk> osVisibleDisksOf(HardwareSpec spec, RaidInventory raidInventory) {
        // 조립 규칙은 OsVisibleDiskRows 가 SSOT(2026-09-17) — RAID 볼륨 행은 멤버 물리 디스크의 종류 · 전송을 쓴다.
        return OsVisibleDiskRows.of(spec, raidInventory);
    }
