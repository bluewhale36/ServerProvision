 ...
 */
public record AxisResolution(AxisOutcome outcome, Long firmwareId, String display, String imagePath, FirmwareAxisReason reason) {
    ...
    public static AxisResolution selected(Long firmwareId, String display, String imagePath) {
        return new AxisResolution(AxisOutcome.SELECTED, firmwareId, display, imagePath, null);
    }

    /** 사유로부터 결과 등급을 받는다 — 사유와 등급이 어긋날 자리를 없앤다. */
    public static AxisResolution of(FirmwareAxisReason reason) {
        return new AxisResolution(reason.getOutcome(), null, null, null, reason);
    }
