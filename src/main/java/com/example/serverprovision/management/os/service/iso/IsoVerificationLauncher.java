...
/**
 * ISO 무결성 검증 Job 시작자. {@link OSMetadataService#verifyIntegrity(Long, Long)} 의 동기 호출을
 ...
 * 같은 클래스 내부 메서드 호출은 프록시를 우회한다. {@link OSMetadataService} 안에 두면 self-proxy
...
public class IsoVerificationLauncher {
	...
	public void runAsync(String jobId, Long osMetadataId, Long isoId) {
		try {
			...
			IntegrityStatus status = osMetadataService.verifyAndRecordIntegrity(osMetadataId, isoId);
