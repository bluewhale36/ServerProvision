...
public interface OSPackageGroupRepository extends JpaRepository<OSPackageGroup, Long> {
	...
	List<OSPackageGroup> findAllByOsMetadata_IdOrderByGroupCode_ValueAsc(Long osMetadataId);

	/**
	 * 추출 upsert 조회 — 동일 (osMetadataId, groupCode) 가 있으면 그 엔티티를 반환.
	 */
	Optional<OSPackageGroup> findByOsMetadata_IdAndGroupCode_Value(Long osMetadataId, String groupCode);
}
