...
public class RhelPostCreationTaskStrategy implements PostCreationTaskStrategy {
	...
	@Override
	public String trigger(OSMetadata osMetadata, ISO iso) {
		String jobId = compsExtractionLauncher.startExtraction(osMetadata.getId(), iso.getId());
		log.info("[S5-7] RHEL ISO 등록 후 comps 추출 자동 트리거. osMetadataId={}, isoId={}, compsJobId={}",
				osMetadata.getId(), iso.getId(), jobId);
		return jobId;
	}
}
