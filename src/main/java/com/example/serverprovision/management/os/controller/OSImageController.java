...
import com.example.serverprovision.management.os.dto.request.*;
import com.example.serverprovision.management.os.dto.response.*;
import com.example.serverprovision.management.os.enums.OSName;
import com.example.serverprovision.management.os.service.*;
...
public class OSImageController {
	...
	public ResponseEntity<?> create(
			@Valid @ModelAttribute("osImageForm") OSImageCreateRequest request,
	...
	public String editForm(@PathVariable Long id, Model model) {
		...
		model.addAttribute(
				"osImageForm", new OSImageUpdateRequest(
						image.osVersion(),
	...
	public ResponseEntity<?> intent(
		...
		//       IsoNudgeRequired / Security) 는 ApiExceptionHandler 가 일괄 처리 (try/catch 제거 — S3-4 정합).
		if (bindingResult.hasErrors()) {
	...
	@ResponseBody
	public OSImageCreateResponse osImageNudgeReplace(
			@PathVariable("nudgeId") java.util.UUID nudgeId,
			@RequestParam(name = "targetId") Long targetId,
			@RequestParam(value = "typedName", required = false) String typedName
	) {
