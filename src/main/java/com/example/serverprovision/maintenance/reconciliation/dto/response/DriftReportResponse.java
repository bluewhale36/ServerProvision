 ...
 *                           스냅샷 도입 이전 행은 서비스 매핑이 미해결 수로 대체한 값
 *                           ({@code DriftReport.getDetectedDriftCountForDisplay()} SSOT)
 * @param driftCount         미해결 드리프트 잔수 (apply/dismiss 로 감소). 배지 "탐지 N · 미해결 M" 의 M
