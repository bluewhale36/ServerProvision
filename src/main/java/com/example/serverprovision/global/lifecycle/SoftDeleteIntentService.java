...
public class SoftDeleteIntentService {
     ...
     */
    public SoftDeleteIntentService(ResourceExistenceChecker resourceExistenceChecker,
    ...
    public <T extends com.example.serverprovision.global.entity.LifecycleEntity & Markable> void checkPrecondition(T entity) {
        ...
        DeleteIntent intent = deleteIntentRegistry.issue(
                ...
                ghostCandidate);
        log.info("[softdelete-reject] intent issued. token={} type={} id={} missingPath={}",
    ...
    public void reconcileThenDelete(ResourceType type, Long resourceId, Runnable normalSoftDelete) {
        for (int attempt = 1; attempt <= RETRY_MAX_ATTEMPTS; attempt++) {
            try {
                ...
                log.info("[softdelete-saga] reconcileThenDelete 성공. type={} id={} attempt={}/{}",
                ...
                if (attempt == RETRY_MAX_ATTEMPTS) {
                    log.error("[softdelete-saga] reconcileThenDelete {}회 재시도 모두 실패. type={} id={}",
                ...
                log.warn("[softdelete-saga] reconcileThenDelete attempt {}/{} 실패. {}ms 후 재시도. msg={}",
