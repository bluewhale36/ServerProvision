...
public enum RedfishError {
    ...
    CONNECT_FAILED("BMC 에 연결하지 못했습니다 — 주소 · 네트워크 · BMC 상태를 확인하세요.", false),
    /** 401 — 자격증명 불일치(다음 후보로 폴백하는 신호). 이 문구는 후보가 소진됐을 때만 사용자에게 닿는다. */
    AUTH_FAILED("모든 자격증명이 거부되었습니다 — 표준 비밀번호와 보드 시리얼(공장 기본)을 확인하세요.", false),
    /** 412 — ETag 선행 조건 불일치(fresh ETag 재시도 신호). */
    PRECONDITION_FAILED("BMC 가 선행 조건(ETag)을 거절했습니다 — 다시 시도하세요.", true),
    /** 404 — 리소스 부재. AMI TaskMonitor 는 작업 종결 후 소멸하므로(실측) 일시 도달 불가와 갈라야 한다. */
    NOT_FOUND("BMC 에 해당 리소스가 없습니다.", true),
    /** 그 외 프로토콜 오류 — 4xx/5xx · 본문 해석 불가. */
    PROTOCOL("BMC 가 요청을 거절했거나 응답을 해석하지 못했습니다.", true);

    @Getter
    private final String userMessage;

    /** 그 리소스만의 거절인가(E2.5 D-4) — 참이면 다음 호출은 성립할 수 있어 best effort 로 삼킬 수 있고,
     * 거짓(연결 · 자격증명)이면 다음 호출도 같은 이유로 실패하므로 그대로 올린다. */
    private final boolean resourceSpecific;

    public boolean resourceSpecific() {
