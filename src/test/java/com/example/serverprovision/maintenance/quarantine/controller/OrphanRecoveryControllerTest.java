...
/**
 * R1-4-4 — 오펀 복구 endpoint 의 사용자 액션 기반 통합 테스트 (URL 불변 회귀).
 * <p>Service 단 mock 만으로는 "도메인 예외 → HTTP status" 매핑(404/409/400)이 드러나지 않으므로,
 * 컨트롤러 + advice 매핑을 함께 실행해 실제 status·body 를 검증한다 (CLAUDE.md 테스트 규율).</p>
 *
 * 시나리오 : 성공(2xx) · 입력검증(400) · 도메인 충돌(409) · 리소스 없음(404).
 */
@WebMvcTest(controllers = OrphanRecoveryController.class)
class OrphanRecoveryControllerTest {

	@Autowired
	MockMvc mvc;

	@MockitoBean
	OrphanQuarantineService quarantineService;
	@MockitoBean
	OrphanRecoveryService recoveryService;
	// @EnableJpaAuditing 이 main class 에 있어 WebMvcTest 부팅 시 jpaMappingContext 를 요구한다.
	@MockitoBean
	JpaMetamodelMappingContext jpaMetamodelMappingContext;

	private static OrphanQuarantineResponse sample(String recoveryId) {
		return new OrphanQuarantineResponse(
				recoveryId, ResourceType.OS_ISO, 1L, "/opt/iso/rocky.iso", "rocky.iso",
				"DB 저장 실패 (제약 위반 등)", "PENDING",
	...
	class PageAndDetail {
		...
		void page_returnsView() throws Exception {
			given(quarantineService.listPending()).willReturn(List.of(sample("r1")));
		...
		void detail_success() throws Exception {
			given(quarantineService.get("r1")).willReturn(sample("r1"));

			mvc.perform(get("/maintenance/quarantine/r1"))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.recoveryId").value("r1"))
					.andExpect(jsonPath("$.resourceType").value("OS_ISO"))
		...
		void detail_notFound() throws Exception {
			given(quarantineService.get("nope"))
	...
	class Retry {
		...
		void retry_success() throws Exception {
			given(recoveryService.retry("r1"))
		...
		void retry_notFound() throws Exception {
			given(recoveryService.retry("nope"))
					.willThrow(new OrphanRecoveryNotFoundException("nope"));
	...
	class Discard {
		...
		void discard_success() throws Exception {
			willDoNothing().given(recoveryService).discard(eq("r1"), eq("rocky.iso"));
		...
		void discard_typedNameMismatch() throws Exception {
			willThrow(new TypedNameMismatchException("rocky.iso", "wrong.iso"))
					.given(recoveryService).discard(eq("r1"), any());
		...
		void discard_notFound() throws Exception {
			willThrow(new OrphanRecoveryNotFoundException("nope"))
					.given(recoveryService).discard(eq("nope"), any());
