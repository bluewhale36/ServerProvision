...
class WindowsDiskSelectionTest {
    ...
    void megaraid_osVolumeFirst_diskZero() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os), inventory(observed("VD0", "spvR1V1", "600605b0aa11")));
    ...
    void megaraid_dataFirst_diskOne() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os),
                inventory(observed("VD0", "data", "600605b0dddd"), observed("VD1", "spvR2V1", "600605b0bb22")));
    ...
    void mptIr_reversedListing_osIsFirstCreated() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os),
                inventory(RaidChipFamily.MPT_IR,
                        ...
                        observed("323", "spvR1V1", "08169c022f61a40c")));
    ...
    void mptIr_osIsSecondCreated_diskOne() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os),
                inventory(RaidChipFamily.MPT_IR,
                        ...
                        observed("323", "spvR1V1", "00a8209837f72f2e")));
    ...
    void nameFallback() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os), inventory(observed("VD0", "spvR1V1", null)));
    ...
    // ==== 표시 걸러내기(F-9) — 번호 계산은 lsblk 를 쓰지 않는다(실기 4호 O-3) ====

    @Test
    @DisplayName("BMC 가상 미디어(USB · 0B)는 디스크 표에 세우지 않는다 — 컨트롤러 디스크만 표시")
    void virtualMedia_notDisplayed() {
        assertThat(WindowsDiskSelection.isControllerDiskForDisplay(disk("sda", "0B", "USB", null))).isFalse();
        assertThat(WindowsDiskSelection.isControllerDiskForDisplay(disk("sdb", "0B", null, null))).isFalse();
        assertThat(WindowsDiskSelection.isControllerDiskForDisplay(disk("sdc", "446.6G", null, "0x600605b0aa11"))).isTrue();
        assertThat(WindowsDiskSelection.isControllerDiskForDisplay(disk("sdd", "14.6T", "SAS", null))).isTrue();
    }
    ...
    void noOsVolume_blocked() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(data), inventory(observed("VD0", "data", "600605b0dddd")));
    ...
    void noInventory_blocked() {
        ...
        assertThat(WindowsDiskSelection.judge(List.of(os), null).wire()).isEqualTo("raid inventory missing");
        assertThat(WindowsDiskSelection.judge(List.of(os), inventory()).wire()).isEqualTo("raid inventory missing");
    }
    ...
    void osVolumeNotInInventory_blocked() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os), inventory(observed("VD0", "other", "600605b0aa11")));
    ...
    void passthroughMixed_blocked() {
        ...
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os, passthrough),
                inventory(observed("VD0", "spvR1V1", "600605b0aa11")));
    ...
    void wwnNormalized() {
        RaidVolume os = volume("spvR1V1", RaidLevel.RAID1, PlannedVolumeRole.OS, "600605b0aa11");
        DiskSelection sel = WindowsDiskSelection.judge(List.of(os), inventory(observed("VD0", "x", "0x600605B0AA11")));
