...
class RhelPostCreationTaskStrategyTest {
    ...
    void triggerRockyDelegates() {
        OSMetadata osMetadata = OSMetadata.builder().osName(OSName.ROCKY_LINUX).osVersion("9.4").build();
        // 테스트용 — @Builder 의 id 는 reflection 없이 직접 set 불가. ID 가 0L 인 mock 으로 진행.
        ISO iso = ISO.builder().osMetadata(osMetadata).isoPath("/opt/iso/rocky/9/dvd.iso").build();

        given(compsExtractionLauncher.startExtraction(any(), any())).willReturn("comps-job-uuid");

        String result = strategy.trigger(osMetadata, iso);

        assertThat(result).isEqualTo("comps-job-uuid");
        verify(compsExtractionLauncher).startExtraction(osMetadata.getId(), iso.getId());
    }

    @Test
    @DisplayName("trigger : CentOS 도 동일하게 위임")
    void triggerCentOsDelegates() {
        OSMetadata osMetadata = OSMetadata.builder().osName(OSName.CENTOS).osVersion("7.9").build();
        ISO iso = ISO.builder().osMetadata(osMetadata).isoPath("/opt/iso/centos/7/dvd.iso").build();
