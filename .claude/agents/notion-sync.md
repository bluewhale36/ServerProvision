...
7. **상위 항목(부모) 재지정** — 계층 재구성 시 자식의 `상위 항목` relation 을 옮긴다. 아래 '계층 단계 구조 규약' 을 따른다.
...
인벤토리 코드는 여러 층의 부모/자식 트리를 이룰 수 있다. 예: `E1-I-2` 아래 `E1-I-2-a`·`E1-I-2-b`, 다시 `E1-I-2-b` 아래 `E1-I-2-b-1`·`E1-I-2-b-2`. 이 구조를 만들고 유지할 때 아래를 지킨다.

- **상위(부모) 단계 페이지는 묶음 의미만 갖는다 — 그 자체로 수행하는 작업 단위가 아니다.** 부모 페이지 본문에는 "상위(부모) 단계로 하위 슬라이스를 묶는 의미만 가지며 자체 수행이 아니다. 세부 실행은 자식 페이지 참조" 를 명시하고, 4항목 대신 **하위 구성(자식 목록과 각 역할) + 완료 조건**을 적는다.
- **자식은 직속 부모에 붙인다.** 하위 슬라이스를 신설할 때 `상위 항목` 을 최상위 우산이 아니라 직속 부모로 설정한다(예: `E1-I-2-b-1` 의 상위 항목은 `E1-I-2-b`). two-way relation 이라 부모의 `하위 항목` 이 자동 갱신됨을 재조회로 확인한다.
- **부모 상태는 자식에서 파생한다(부모 스스로 진행하지 않는다).**
  - 자식 중 하나라도 `진행 중` 이면 → 부모 `진행 중`.
  - 자식 전부 `완료` 면 → 부모 `완료`.
  - 그 외(자식 전부 `시작 전`) → 부모 `시작 전`.
